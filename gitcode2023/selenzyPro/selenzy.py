...
class preLoad(object):
    ...
    def fpData(self, datadir):
        ...
        self.fpr = {}    
        for fpid in quickRsim.fingerprint():
            self.fpr[fpid] = quickRsim.loadFingerprintRF(datadir, 'FP_MorgRF')
...
def seqScore(newscore=None):
    ...
    vdict = {
        string.ascii_uppercase[10]: ('Reaction similarity:', 50.0, True),
        string.ascii_uppercase[13]: ('Combined score', 0.0, False),
        string.ascii_uppercase[11]: ('RF similarity', 50.0, True),
        ...
        string.ascii_uppercase[8]: ('Uniprot <a href="http://www.uniprot.org/help/protein_existence" target="_blank">protein evidence</a>:', -0.1, True),
        string.ascii_uppercase[14]: ('Percentage helices:', 0.0, False),
        ...
        string.ascii_uppercase[20]: ('Percentage polar amino acids:', 0.0, False)
    }
    
    nvdict = vdict.copy()
    # Reference order (alternatively, perhaps easier to keep table order)
    clist = [string.ascii_uppercase[x] for x in [10, 11, 4, 8, 13, 9, 14, 15, 16, 18, 19, 20]]
...
def sort_rows_df(rows, filt, head = ['Seq. ID', 'Description', 'Organism Source', 'Tax. distance', 
                                     'Rxn. ID','Rxn. ID - additional', 'EC Number', 
                                     'Uniprot protein evidence','Consv. Score', 'Rxn Sim.',
                                     'Rxn Sim RF.', 'Direction Used', 'Combined score', 
                                     '% helices', '% sheets', '% turns', '% coils', 'Mol. Weight', 
                                     'Isoelec. Point','Polar %', 'Query', 'Hit']):
...
def analyse(rxnInput, targ, datadir, outdir, csvfilename, pdir=0, host='83333', NoMSA=True, pc=None, fp='Morgan'):
    ...
    with open(outdir+'_report.txt', 'a+') as handler:
...
#    csvname = rxnname.rsplit('/', 1)[-1]
    ...
    for x in list_mnx1:
        if MnxSimRF[x] != 'nan':
            if x in MnxToUprot:
                ...
                if len(top_reactions1) == 10: 
                    break
    for x in top_reactions1: 
        print(x, MnxSimRF[x], str(round(float(MnxSimRF[x]), 2)), MnxSim[x])
    ...
    for x in list_mnx2:
        if MnxSim[x] != 'nan':
            if x in MnxToUprot:
                ...
                if len(top_reactions2) == 10: 
                    break
    for x in top_reactions2: 
        print(x, MnxSim[x], str(round(float(MnxSim[x]), 2)), MnxSimRF[x])    
        ...
        if up is not None:
            for y in up:
                if len(targets) >= int(targ):    # allow user to manipulate desired number of entries for resulting table
                    ...
                    # allow multiple reactions to be associated with enzymes
    ...
    for y in targets:
        try:
            ...
            rows.append( (y, desc, org, tdist[org], mnx, mnxOther, ecid, ext, conservation, rxnsim, mnxSimRF, rxndirused, avScore, h, e, t, c, w, i, pol, Smiles, mnxSmiles) )
...
if __name__ == '__main__':
    ...
    if not os.path.exists(newpath):
        os.makedirs(newpath)
        with open(os.path.join(arg.datadir)+'report.txt', 'w+') as f: 
            f.write('new file\n')
