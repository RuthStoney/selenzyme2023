...
def init_session():
    ...
    app.logger.info( 'innit session: %s' % (uniqueid,) )
    uniquefolder = os.path.join(app.config['UPLOAD_FOLDER'], uniqueid)
...
def run_session(rxntype, rxninfo, targets, direction, host, fp, noMSA, frag_size=7):
    global session
    print('in run_session, frag_size', frag_size)
    uniqueid = session['uniqueid']
...
class RestGate(Resource):
    ...
    def get(self):
        return {'app': 'Selenzy', 'version': '1.3', 'author': 'Synbiochem'}
...
class RestQuery(Resource):
    ...
    def post(self):
        ...
        print('frag_size restquery', frag_size)
...
class RestSource(Resource):
    ...
    def get(self):
        ...
        return jsonify({'app': 'Selenzy', 'version': '1.3', 'author': 'Synbiochem', 'data': orgs})
...
if __name__== "__main__":  #only run server if file is called directly
    ...
    print('frag_size main', arg.frag_size)
